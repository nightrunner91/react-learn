...
Все стрелки замыкают одну и ту же переменную `i`. К моменту вызова цикл уже завершился.
...
Все методы разделяют одно замыкание с переменной `count`.
...
Внутри `bar` есть локальная переменная `a`, объявленная через `let`. До её инициализации переменная находится в TDZ, поэтому даже чтение `a` из внешнего окружения невозможно.
...
Каждый вызов `add` создаёт замыкание с собственным значением `a`.
...
`let` создаёт новую переменную на каждой итерации, поэтому каждая стрелочная функция замыкает собственное значение `i`.
