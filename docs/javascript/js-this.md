...
**Ответ:** `undefined` (или имя глобального объекта, если оно есть).

Метод извлечён из объекта и вызван без контекста.
...
Стрелочная функция создана в глобальном контексте и не получает `this` от `obj`.
...
**Ответ:** `undefined`, `"Alice"`.

Без `new` функция возвращает `undefined`, а `this.name` записывается в глобальный объект.
...
function callWithDelay(callback, ms) {
  setTimeout(callback, ms);
}

callWithDelay(user.greet.bind(user), 100);
```

**Ответ:** `"Hi, Bob"`.

`bind` привязывает `this` к `user` навсегда.
...
**Ответ:** `undefined: Alice`, `undefined: Bob`.

Внутри `forEach` обычная функция вызывается без контекста. `this` теряется.

**Исправление через стрелочную функцию:**
...
`bind` может частично применять аргументы. Первый аргумент (`null`) — это `this`, остальные фиксируются.
